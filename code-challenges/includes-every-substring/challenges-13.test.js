...
const onlyOddChars = (str) => {
  return str.split('').filter((val, index) => index % 2).join('');
};
...
const unenrollBrook = (arr) => {
  return arr.map(currentArray => currentArray.filter(string => !string.includes('Brook')));
};
...
const sortByDay = (arr) => {
  let result = new Array(daysOfWeek.length).fill([]);
  for(let i = 0; i < daysOfWeek.length; i++){
    result[i] = arr.filter(val => val.includes(daysOfWeek[i]));
  }
  return result;
};
...
const characterByIndex = (arr) => {
  return arr.map((val, index) => val.charAt(index));
};
