...
const count = (target, input) => {
  return input.reduce((total, curArray) => {
    let firstTotal = curArray.reduce((curTotal, val) => {
      if(val === target){
        curTotal += 1;
      }
      return curTotal;
...
const totalSum = (input) => {
  return input.reduce((total, curArray) => {
    let first = curArray.reduce((curTotal, curVal) => {
      return curTotal += curVal;
    });
    return total + first;
  }, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(numArray => {
    let filtered = numArray.filter(curVal => typeof(curVal) === 'number' && (curVal % 5) === 0);
...
let findMaleAndFemale = (data) => {
  return data.reduce((result, character) => {
    if (character.gender === 'male' || character.gender === 'female'){
      if(result !== ''){
        result = `${result} and ${character.name}`;
      } else{
...
let findShortest = (data) => {
  let result = data.reduce((shortest, character) => {
    if(parseInt(shortest.height) > parseInt(character.height)){
      shortest.char = character.name;
      shortest.height = character.height;
    }
    return shortest;
  },{char:'', height: 100000});
